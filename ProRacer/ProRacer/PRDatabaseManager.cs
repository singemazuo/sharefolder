...
namespace ProRacer
{
    ...
    public class PRDatabaseManager
    {
        ...
        {
            SqlCommand cmmd = new SqlCommand("SELECT *, SponsorId AS FK_SponsorId, FirstName + ',' + LastName AS [FullName] FROM Participant ORDER BY FirstName, LastName", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(cmmd);
        ...
        {
            SqlCommand cmmd = new SqlCommand("SELECT *,Sponsor.SponsorId AS FK_ParticipantId,Participant.ParticipantId AS PK_ParticipantId, FirstName + ',' + LastName AS [FullName] FROM Participant LEFT JOIN Sponsor ON Participant.SponsorId = Sponsor.SponsorId ORDER BY FirstName, LastName", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(cmmd);
        ...
        {
            SqlCommand cmmd = new SqlCommand("SELECT * FirstName + ',' + LastName AS [FullName] FROM Sponsor LEFT JOIN Sponsor ON Participant.SponsorId = Sponsor.SponsorId ORDER BY FirstName, LastName", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(cmmd);
            adapter.Fill(ds, name);
        }
