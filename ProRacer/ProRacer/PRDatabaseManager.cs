...
namespace ProRacer
{
    ...
    public class PRDatabaseManager
    {
        ...
        {
            SqlCommand cmmd = new SqlCommand("SELECT *,PK_ParticipantId, FirstName + ',' + LastName AS [FullName] FROM Participant LEFT JOIN Sponsor ON Participant.SponsorId = Sponsor.SponsorId ORDER BY FirstName, LastName", conn);
            SqlDataAdapter adapter = new SqlDataAdapter(cmmd);
